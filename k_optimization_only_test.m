...
%save(sprintf('./dataset/channel_dataset_Nt_%d_B_%d_T_%d_K_%d_num_%d.mat',Nt,B,T,K,data_number),'channel_dataset');
%load(sprintf('./dataset/channel_dataset_Nt_%d_B_%d_T_%d_K_%d_num_%d.mat',Nt,B,T,K,data_number));
load('test_data.mat');
%% find the pareto bound
beta_diff = 0.05;
data_number = 10;
SNR_list = 0:5:20; T_list = [1]; K_list = [1,2,3,4,5];
SNR_list = [1]; T_list = [3]; K_list = [5];
...
for t_idx = 1:length(T_list)
    ...
    for idx = 1:length(SNR_list)
       ...
       %% Problem solve
        for i = 1:data_number
            i=17;
            channel_instance = squeeze(channel(i,:,:));
            %% SCA_FP initation
            % initiation
            %precoding_matrix = squeeze(precoding_matrix_dataset(i,:,:));
            precoding_matrix_init = squeeze(precoding_matrix_init(17,:,:));
            %precoding_matrix_init = SINR_balance_beamforming_v3(channel_instance,B,T,L,0,M,a,b,c,alpha,ones(B,1),sigma,1e-3);
            %load('precoding_init.mat'); precoding_matrix_init = squeeze(precoding_matrix_init(1,:,:));
            ...
            for K = 1:length(K_list)
                ...
                %% SCA FP  
                if qos(1) >= beta_diff
                    [precoding_matrix,object_performance_list,cvx_obj_list,solving_result] = SCA_FP_v4(channel_instance,precoding_matrix_init,B,T,L,J,M,a,b,c,alpha,ones(B,1),sigma);
                else
