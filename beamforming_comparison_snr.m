...
MM_FP_performance_list = zeros(size(SNR_list));
LP_MM_FP_performance_list = zeros(size(SNR_list));
%data_number = 1;
for idx = 1:length(SNR_list)
    ...
    MM_FP_instance_performance_list = zeros(data_number,1);
    LP_MM_FP_instance_performance_list = zeros(data_number,1);
   %% Problem solve
    for i = 1:data_number
        i
       ...
       %% SCA_FP initation
        ...
        toc
        MM_FP_instance_performance_list(i) = mean(object_performance_list(:,end));
        
        tic
        precoding_matrix = MM_FP_naive_v1(channel_instance,B,T,L,J,M,a,b,c,alpha,beta,sigma);
        toc
        [object_performance,qos,power] = performance(channel_instance(:,1:B),channel_instance(:,B+1:B+T),precoding_matrix(:,1:B),precoding_matrix(:,B+1:B+T),sigma,a,b,c,alpha,L,J,M,B,T);  
        LP_MM_FP_instance_performance_list(i) = mean(object_performance); 
    end
    ...
    MM_FP_performance_list(idx) = mean(MM_FP_instance_performance_list);
    LP_MM_FP_performance_list(idx) = mean(LP_MM_FP_instance_performance_list);
end
...
plot(SNR_list,MM_FP_performance_list,'-+','linewidth',1.5,'Color','#FFE4E1','MarkerSize',10); hold on;
plot(SNR_list,LP_MM_FP_performance_list,'-*','linewidth',1.5,'Color','#77AC30','MarkerSize',10); hold on;
set(gca,'linewidth',1,'fontsize',16,'fontname','Times');
le = legend('MRT-PC','ZF-PC','WMMSE-PC','MM-FP','LP-MM-FP'); set(le,'linewidth',1,'fontsize',16,'fontname','Times New Roman');
grid on;
