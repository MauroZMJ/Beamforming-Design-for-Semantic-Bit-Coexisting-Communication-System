function precoding_matrix = QOS_PowerControl_v4(channel,precoding_matrix,B,T,L,J,M,a,b,c,alpha,beta,sigma)
    ...
    if sum(qos>= beta) == 5
        while end_condition == 0
            ...
            [p_bit,p_sem,solving_result] = update_precoding_matrix_lp(bituser_channel,semuser_channel,bituser_precoding,semuser_precoding,hv_matrix,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,L,J,M,a,b,c,alpha);
            ...
            if abs(mean(object_performance)-last_performance) <= 1e-3 && max(abs(qos-beta)) <= 2e-2 || iteration > 20
                end_condition = 1;
                % [gamma,y, z] = update_sinr_term(hv_matrix,p_bit,p_sem,sigma,B,T);
                % [x,m, n] = update_fraction_variable(hv_matrix,p_bit,p_sem,sigma,gamma,y,z,B,T,a,b,c,alpha);
            ...
            mean(object_performance)
            max(abs(qos-beta)) <= 2e-2
        end
    else
...
function [x,m,n] = update_fraction_variable(hv_matrix,p_bit,p_sem,sigma,gamma,y,z,B,T,a,b,c,alpha)
    ...
    %update x
    for t = 1:T
        interference = G(t)*sigma*power+F(t)*p_sem(t)*pow_abs(hv_matrix(B+t,B+t),2);
        for i = 1:T
            if i ~=t
...
function [reconstruct_bituser_precoding,reconstruct_semuser_precoding,inverse_bituser_matrix,inverse_semuser_matrix]=precoding_recovery(hv_matrix,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,alpha,a,b,c,L,J,M,lamda,mu)
    ...
    for this_inverse_sem_user = 1:T
        ...
        for sem_idx = 1:T
            if sem_idx ~=this_inverse_sem_user
            slope = E(sem_idx)*G(sem_idx);
            scalar = slope * x(sem_idx)^2;
            %eye_scalar = eye_scalar + scalar;
    ...
    reconstruct_semuser_precoding = real(reconstruct_semuser_precoding);
    reconstruct_bituser_precoding = real(reconstruct_bituser_precoding);
end
...
function lambda = update_lambda_new(hv_matrix,p_bit,p_sem,lambda_old,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,L,J,M,a,b,c,alpha)%hv_matrix,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,L,J,M,a,b,c,alpha
    b_aug = b;
    [D,E,F,G] = cal_scalar(a,b_aug,c,alpha,gamma,T);
    p_bit = p_bit.^2;
    p_sem = p_sem.^2;
    ...
    for b = 1:B
        ...
        qos_list(b) = J/M*(log2(1+y(b))-y(b)-m(b)^2*interference_m);
        qos_list(b) = qos_list(b) + (L-J)/M*(log2(1+z(b))-z(b)-n(b)^2*interference_n);
        qos_list(b) = qos_list(b) - beta(b);
    end
    for bit_idx = 1:B
        scalar = J/M*2*m(bit_idx)*sqrt(p_bit(bit_idx)*(1+y(bit_idx)))*real(hv_matrix(bit_idx,bit_idx)) +(L-J)/M*2*n(bit_idx)*sqrt(p_bit(bit_idx)*(1+z(bit_idx)))*real(hv_matrix(bit_idx,bit_idx));
        lambda(bit_idx) = -qos_list(bit_idx)/(scalar/lambda_old(bit_idx));
    end
end

function [bituser_precoding,semuser_precoding,solving_result,obj] = update_precoding_matrix_lp(bituser_channel,semuser_channel,bituser_precoding,semuser_precoding,hv_matrix,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,L,J,M,a,b,c,alpha)
    ...
    while sum(abs(reconstruct_lamda-reconstruct_lamda_old))>1e-4
        reconstruct_lamda_old = reconstruct_lamda;
        %test
        %load('lambda.mat'); reconstruct_lamda=lambda;reconstruct_lamda_old = reconstruct_lamda;p_bit = p_bit'; p_sem=p_sem';%p_bit=sqrt(p_bit); p_sem=sqrt(p_sem);
        %cal_obj_value(hv_matrix,p_bit,p_sem,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,alpha,a,b,c,L,J,M,reconstruct_lamda,reconstruct_mu);
        %[reconstruct_bituser_precoding,reconstruct_semuser_precoding,inverse_bituser_matrix,inverse_semuser_matrix] = precoding_recovery_vector(bituser_channel,semuser_channel,bituser_precoding,semuser_precoding,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,alpha,a,b,c,L,J,M,reconstruct_lamda,reconstruct_mu);
        ...
        %[p_bit,p_sem,inverse_bituser_matrix,inverse_semuser_matrix]=precoding_recovery(hv_matrix,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,alpha,a,b,c,L,J,M,reconstruct_lamda,reconstruct_mu);
        %cal_obj_value(hv_matrix,p_bit,p_sem,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,alpha,a,b,c,L,J,M,reconstruct_lamda,reconstruct_mu);
        ...
        reconstruct_lamda = update_lambda_new(hv_matrix,reconstruct_bituser_precoding,reconstruct_semuser_precoding,reconstruct_lamda_old,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,L,J,M,a,b,c,alpha);
        iter = iter + 1;
        if iter > max_iter 
            break;
        end
    ...
    %obj = sum(obj_list);
    %reconstruct_lamda
    %sum(abs(reconstruct_lamda-reconstruct_lamda_old))>1e-4
    power_sum = (sum(bituser_precoding.^2)+sum(semuser_precoding.^2));
    bituser_precoding = real(bituser_precoding).^2/power_sum;
    semuser_precoding = real(semuser_precoding).^2/power_sum;
end
...
function [p_bit,p_sem,solving_result,obj_value] = update_precoding_matrix_sum(hv_matrix,sigma,Nt,B,T,gamma,y,z,x,m,n,beta,L,J,M,a,b,c,alpha)
    ...
    % p_bit = sqrt(p_bit);
    % p_sem = sqrt(p_sem);
    power_sum = (sum(p_bit)+sum(p_sem));
    p_bit = p_bit/power_sum;
    p_sem = p_sem/power_sum;
    %obj_list
end