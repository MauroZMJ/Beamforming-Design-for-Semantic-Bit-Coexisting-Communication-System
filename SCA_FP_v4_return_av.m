function [precoding_matrix,object_performance_list,cvx_obj_list,solving_result,Gamma,y,z,x,m,n] = SCA_FP_v4_return_av(channel,precoding_matrix,B,T,L,J,M,a,b,c,alpha,beta,sigma)
...
function [x,m,n] = update_fraction_variable(bituser_channel,semuser_channel,bituser_precoding,semuser_precoding,sigma,gamma,y,z,a,b,c,alpha,B,T)
    ...
    for t = 1:T
        this_user_channel = semuser_channel(:,t);
        interference = coef(t)*sigma*power+c*alpha*gamma(t)^(alpha-1)*abs(this_user_channel'*semuser_precoding(:,t)).^2;
        for i = 1:T
            if i~=t
