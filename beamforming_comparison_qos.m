...
B = 5; T = 3; Nt = 16;
data_number  = 1000; %number of test examples.
...
MM_FP_close_form_performance_list = zeros(size(beta_list));
LP_MM_FP_performance_list = zeros(size(beta_list));
...
for idx = 1:length(beta_list)
   ...
   %% Problem solve
    for i = 1:data_number
        ...
        %% MM_FP initation
        % initiation (This operation is not necessary, but it can garantee the feasible point at initialization.)        
    ...
    MM_FP_close_form_performance_list(idx) = mean(MM_FP_instance_performance_list);
...
plot(beta_list,MM_FP_close_form_performance_list,'-+','linewidth',1.5,'Color','#FFE4E1','MarkerSize',10); hold on;
plot(beta_list,LP_MM_FP_performance_list,'-*','linewidth',1.5,'Color','#77AC30','MarkerSize',10); hold on;
