function [precoding_matrix,object_performance_list,cvx_obj_list,solving_result] = MM_FP(channel,precoding_matrix,B,T,L,J,M,a,b,c,alpha,beta,sigma)
...
function [D,E,F,G] = cal_scalar(a,b,c,alpha,gamma_0,T)
    if alpha <= 1
        ...
        E = alpha*b*gamma_0.^(alpha-1)./slope;
        F = alpha*c*gamma_0.^(alpha-1);
        G = slope;
    end
end
...
function [x,m,n] = update_fraction_variable(bituser_channel,semuser_channel,bituser_precoding,semuser_precoding,sigma,gamma,y,z,a,b,c,alpha,B,T)
    ...
    for t = 1:T
        this_user_channel = semuser_channel(:,t);
        interference = G(t)*sigma*power+F(t)*abs(this_user_channel'*semuser_precoding(:,t)).^2;
        for i = 1:T
            if i~=t
                interference = interference + G(t)*abs(this_user_channel'*semuser_precoding(:,i)).^2;
            end
        end
        for i = 1:B
            interference = interference + G(t)*abs(this_user_channel'*bituser_precoding(:,i)).^2;
        end
        x(t) = abs(real(this_user_channel'*semuser_precoding(:,t))) / interference;
    end
    %update m
