...
%[channel_dataset,precoding_matrix_dataset] = generate_dataset(data_number,Nt,B,T,10,L,J,M,a,b,c,alpha,ones(B,1),1);
...
%SNR_list = -5:1:10;
SNR_list = [0,2];
MRT_performance_list = zeros(size(SNR_list));
ZF_performance_list = zeros(size(SNR_list));
WMMSE_performance_list = zeros(size(SNR_list));
SCA_FP_Lower_bound_performance_list = zeros(size(SNR_list));
SCA_FP_performance_list = zeros(size(SNR_list));
SCA_FP_close_form_performance_list = zeros(size(SNR_list));
MM_FP_naive_performance_list = zeros(size(SNR_list));
...
for idx = 1:length(SNR_list)
    ...
    SCA_FP_Lower_bound_instance_performance_list = zeros(data_number,1);
   ...
   %% Problem solve
    for i = 1:data_number
       ...
       %% SCA_FP initation
        ...
        if qos(1) >= beta(1)
            %precoding_matrix = squeeze(precoding_matrix_dataset(i,:,:));
            [precoding_matrix,object_performance_list,cvx_obj_list,solving_result] = MM_FP(channel_instance,precoding_matrix_init,B,T,L,J,M,a,b,c,alpha,beta,sigma);
            %[precoding_matrix,object_performance_list,cvx_obj_list,solving_result,gamma,y,z,x,m,n] = SCA_FP_v4_return_av(channel_instance,precoding_matrix_init,B,T,L,J,M,a,b,c,alpha,beta,sigma);
        else
        ...
        %only_for_test(channel_instance,precoding_matrix_init,B,T,L,J,M,a,b,c,alpha,beta,sigma,gamma,y,z,x,m,n)

        % [precoding_matrix,object_performance_list,cvx_obj_list,solving_result] = SCA_FP_lb(channel_instance,precoding_matrix_init,B,T,L,J,M,a,b,c,alpha,beta,sigma);
        ...
        toc
        SCA_FP_Lower_bound_instance_performance_list(i) = mean(object_performance_list(:,end));
...
%         SCA_FP_instance_performance_list(i) = mean(object_performance_list(:,end));
    ...
    SCA_FP_Lower_bound_performance_list(idx) = mean(SCA_FP_Lower_bound_instance_performance_list);
...
plot(SNR_list,MRT_performance_list,'-s','linewidth',1.5,'Color','#0072BD','MarkerSize',10); hold on;
plot(SNR_list,ZF_performance_list,'-d','linewidth',1.5,'Color','#D95319','MarkerSize',10); hold on;
plot(SNR_list,WMMSE_performance_list,'-^','linewidth',1.5,'Color','#EDB120','MarkerSize',10); hold on;
plot(SNR_list,SCA_FP_Lower_bound_performance_list,'-.','linewidth',1.5,'Color','#7E2F8E','MarkerSize',10); hold on;
plot(SNR_list,SCA_FP_performance_list,'-*','linewidth',1.5,'Color','#77AC30','MarkerSize',10); hold on;
plot(SNR_list,SCA_FP_close_form_performance_list,'-+','linewidth',1.5,'Color','#FFE4E1','MarkerSize',10); hold on;
plot(SNR_list,MM_FP_naive_performance_list,'-+','linewidth',1.5,'MarkerSize',10); hold on;
set(gca,'linewidth',1,'fontsize',16,'fontname','Times');
le = legend('MRT-PC','ZF-PC','WMMSE-PC','SCA-FP-Maxmin','SCA-FP','SCA-FP-CF','MM-FP-naive'); set(le,'linewidth',1,'fontsize',16,'fontname','宋体');
grid on;
