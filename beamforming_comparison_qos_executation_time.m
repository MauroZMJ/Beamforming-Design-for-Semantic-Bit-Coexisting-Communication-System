...
MRT_time_list = zeros(size(beta_list));
ZF_time_list = zeros(size(beta_list));
WMMSE_time_list = zeros(size(beta_list));
SCA_FP_Maxmin_time_list = zeros(size(beta_list));
SCA_FP_time_list = zeros(size(beta_list));
SCA_FP_close_form_time_list = zeros(size(beta_list));
MM_FP_naive_time_list = zeros(size(beta_list));
%data_number = 1;
for idx = 1:length(beta_list)
    ...
    MRT_instance_time_list = zeros(data_number,1);
    ZF_instance_time_list = zeros(data_number,1);
    WMMSE_instance_time_list = zeros(data_number,1);
    SCA_FP_instance_time_list = zeros(data_number,1);
    SCA_FP_Maxmin_instance_time_list = zeros(data_number,1);
    SCA_FP_close_form_instance_time_list = zeros(data_number,1);
    MM_FP_naive_instance_time_list = zeros(data_number,1);
   %% Problem solve
    for i = 1:data_number
        i
       ...
       %% WMMSE precoding
        ...
        WMMSE_instance_time_list(i) = toc;
        [object_performance,qos,power] = performance(channel_instance(:,1:B),channel_instance(:,B+1:B+T),precoding_matrix(:,1:B),precoding_matrix(:,B+1:B+T),sigma,a,b,c,alpha,L,J,M,B,T);
       ...
       %% SCA_FP initation
        ...
        SCA_FP_close_form_instance_time_list(i)=toc;
        ...
        MM_FP_naive_instance_time_list(i) = toc;
        [object_performance,qos,power] = performance(channel_instance(:,1:B),channel_instance(:,B+1:B+T),precoding_matrix(:,1:B),precoding_matrix(:,B+1:B+T),sigma,a,b,c,alpha,L,J,M,B,T);  
...
%         SCA_FP_instance_performance_list(i) = mean(object_performance_list(:,end));
    ...
    MRT_time_list(idx) = mean(MRT_instance_time_list);
    ZF_time_list(idx) = mean(ZF_instance_time_list);
    WMMSE_time_list(idx) = mean(WMMSE_instance_time_list);
    SCA_FP_Maxmin_time_list(idx) = mean(SCA_FP_Maxmin_instance_time_list);
    SCA_FP_time_list(idx) = mean(SCA_FP_instance_time_list);
    SCA_FP_close_form_time_list(idx) = mean(SCA_FP_close_form_instance_time_list);
    MM_FP_naive_time_list(idx) = mean(MM_FP_naive_instance_time_list);
end
